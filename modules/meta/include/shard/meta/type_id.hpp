...
private:
    static std::atomic<T> s_counter;
};

template <typename T, typename U>
std::atomic<T> typespace<T, U>::s_counter = ATOMIC_VAR_INIT(0);
...
struct std::hash<shard::meta::type_id<T, U>> {
    std::size_t operator()(const shard::meta::type_id<T, U>& type_id) const noexcept {
...
#define SHARD_INTERNAL_TYPEID_2(I, T) shard::meta::typespace<I::value_type, I>::id<T>()
