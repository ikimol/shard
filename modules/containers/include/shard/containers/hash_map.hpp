...
#ifndef SHARD_CONTAINERS_HASH_MAP_HPP
#define SHARD_CONTAINERS_HASH_MAP_HPP

#include <shard/containers/array.hpp>
#include <shard/core/preprocessor.hpp>
...
///
/// \param h The hash value as returned by the hasher
/// \param bc The number of buckets
...
private:
    struct entry {
        ...
        template <typename U, typename = std::enable_if_t<!std::is_same<U, entry>::value>>
        explicit entry(U&& key) : key(SHARD_FWD(key)) {}

        /// Forwarding key & value constructor
        template <typename U, typename V>
        entry(U&& key, V&& value) : key(SHARD_FWD(key)), value(SHARD_FWD(value)) {}
...
private:
    struct node {
        ...
        explicit node(Args&&... args) : entry(SHARD_FWD(args)...) {}
    ...
    public:
        ...
        explicit basic_iterator(hash_map* map, bool end = false) noexcept : m_map(map) {
            if (end) {
        ...
        basic_iterator(hash_map* m, node* n, size_type b) noexcept : m_map(m), m_node(n), m_bucket(b) {}
...
public:
    ...
    explicit hash_map(allocator& a) : m_allocator(&a), m_buckets(a) {}

    /// Create a new hash map and allocate memory for \a bc buckets
    hash_map(allocator& a, size_type bc) : m_allocator(&a), m_buckets(a, bc) {}
    ...
    hash_map& operator=(hash_map&& other) noexcept { return *this; }
    ...
    std::pair<iterator, bool> emplace(Args&&... args) {
        ...
        // auto& key = std::get<0>(std::tuple<Args...>(args...));
        auto& key = first_arg(SHARD_FWD(args)...);
        if (bc != 0) {
        ...
        auto b = detail::constrain_hash(m_hasher(key), bc);
        auto n = new_object<node>(*m_allocator, SHARD_FWD(args)...);
        n->next = m_buckets[b];
        m_buckets[b] = n;
        ++m_size;
    ...
    /// container
    void reserve(size_type n) { rehash(static_cast<size_type>(std::ceil(n / m_max_load_factor))); }
...
#endif // SHARD_CONTAINERS_HASH_MAP_HPP
