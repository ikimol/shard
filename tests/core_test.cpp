...
TEST_SUITE("core") {
    ...
    TEST_CASE("byte") {
        ...
        CHECK(shard::to_integer<int>(b) == 2);

        const auto b1 = shard::to_byte(0b0011);
        const auto b2 = shard::to_byte(0b1010);

        SUBCASE("integer value") {
            CHECK(shard::to_integer<int>(b1) == 3);
            CHECK(shard::to_integer<int>(b2) == 10);
        }

        SUBCASE("binary operations") {
            CHECK(shard::to_integer<int>(b1 | b2) == 11);
            CHECK(shard::to_integer<int>(b1 & b2) == 2);
        }

        SUBCASE("usage as buffer") {
            shard::byte buffer[22];
            auto byte_out = buffer_manipulator((unsigned char*)(buffer), 22);
            CHECK(byte_out == 20);
            CHECK(shard::to_integer<int>(buffer[0 * sizeof(int)]) == 0);
            CHECK(shard::to_integer<int>(buffer[1 * sizeof(int)]) == 1);
            CHECK(shard::to_integer<int>(buffer[2 * sizeof(int)]) == 2);
            CHECK(shard::to_integer<int>(buffer[3 * sizeof(int)]) == 3);
            CHECK(shard::to_integer<int>(buffer[4 * sizeof(int)]) == 4);
        }
    }
}